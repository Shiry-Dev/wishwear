...
function ProductScreen() {
  ...
  const addToCartHandler = async () => {
    ...
    if (data.countInStock < quantity) {
      window.alert(':( Produto não disponível no momento ):');
      return;
    }
    ctxDispatch({
      type: 'CART_ADD_ITEM',
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    if (!comment || !rating) {
      toast.error('Entre com um review e nota.');
      return;
    }
    try {
      ...
      toast.success('Review feito com sucesso');
      ...
      <div className="my-3">
        ...
        <div className="mb-3">
          {product.reviews.length === 0 && (
            <MessageBox>Sem reviews no momento...</MessageBox>
          )}
        </div>
        <ListGroup>
